...
#define SERVER_STRING "Server: http/1.1\r\n"
...
// making header for HTTP response
...
int main(int argc, char *argv[])
{
 ...
 while (1) {
   ...
   enq(client_sock);
   pthread_cond_broadcast(&workers); //broadcasting worker threads to wake up and check queue
   pthread_mutex_unlock(&mutex);
 }
